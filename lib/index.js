...
Laynode.start = function(req,res,a,m) {
    var laynode = Laynode.getInstance();
        laynode.run(req,res,a,m);
};
...
Laynode.prototype.run = function(req,res,a,m) {
    var st = new Date().getTime();
    var parser  = url.parse(req.url,true);
    var actionName = ('string' == typeof a && a)?a:parser.pathname.substr(1);
    ...
    if(actions[actionName] && actions[actionName]['classname']) {
        ...
        actionObj.on('init',function() {
            ...
            console.log('event dispatch');console.log(method);
