...
var logger      = tracer.dailyfile({root : rootpath + '/logs/', format : "{{timestamp}} <{{title}}> {{file}}:{{line}} ({{method}}) {{message}}", dateformat : "yyyy-mm-dd HH:MM:ss.L"});

global._laynode_basepath    = basepath;//string 非引用
global._laynode_rootpath    = rootpath;//string 非引用
global._laynode_logger      = logger;
global.logger               = logger;
global.basepath             = basepath;
global.rootpath             = rootpath;

//logger.log('global.logger', global.logger);
...
Laynode.prototype.run = function(req,res,a,m,params) {
    ...
    if(actions[actionName] && actions[actionName]['classname']) {
        ...
        logger.log('Into object{' + actionObj._classname + '} constructor   ', 'No.' + actionObj.num);
        actionObj.on('init',function() {
            logger.log('Into object{' + actionObj._classname + '} event init    ', 'No.' + actionObj.num);
            ...
            logger.log('Into object{' + actionObj._classname + '} event dispatch', 'No.' + actionObj.num, 'dispatch method is ' + method);
            actionObj[method].call(actionObj);
        }).on('launch',function() {
            logger.log('Into object{' + actionObj._classname + '} event launch  ', 'No.' + actionObj.num);
            actionObj.end.call(actionObj);
        }).on('end',function() {
            logger.log('Into object{' + actionObj._classname + '} event end     ', 'No.' + actionObj.num);
            var et = new Date().getTime();
            logger.log('Success == st: ' + st + ' ---- et: ' + et);
            logger.log('It creates ' + Base.prototype.instances + ' instances');
            res.end();
        }).on('error',function(err) {
            logger.log('Into object{' + actionObj._classname + '} event error   ', 'No.' + actionObj.num);
            logger.log(err);
            var et = new Date().getTime();
            logger.log('Error ==== starttime: ' + st + ' ---- endtime: ' + et);
            res.end('error');
        });
        //action init
